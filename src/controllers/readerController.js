...
const readerController = {
    ...
    getById: async (req, res) => {
        try {
            ...
            if (!reader) return res.status(404).json({ success: false, message: 'Lector no encontrado' });
    ...
    update: async (req, res) => {
        try {
            const reader = await Reader.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
            if (!reader) return res.status(404).json({ success: false, message: 'Lector no encontrado' });
            res.json({ success: true, data: reader });
        } catch (error) {
            res.status(400).json({ success: false, message: error.message });
